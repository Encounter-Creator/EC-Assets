...
begin
  ...
  if v_normalized_outcome not in ('stationed', 'available', 'damaged') then
  ...
  set
    ...
    status = case
      when v_normalized_outcome = 'damaged' then 'damaged'
      when v_normalized_outcome = 'available' then 'available'
      else 'stationed'
    end
