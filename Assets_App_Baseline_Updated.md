...
This is the current baseline draft based on the decisions locked so far. It is intentionally incomplete and will be expanded after the remaining question rounds.
...
- Can change `name`
- Can change `tag` with duplicate warnings
- Can change `department/team`
- Cannot change `serial number`
- Cannot edit main notes/description
- After resolving a damage case, a scoped one-time condition note field becomes available on the asset for the resolving manager. This does not grant general notes/description editing rights — it is a post-resolution condition record only.
...
- `Returns` required fields are `Return Date`, `Return Location`, and `Note`
...
- `Approvals > Returns` is the decision surface. User-submitted return requests appear here first. The manager reviews, accepts or declines, and if accepting, chooses the destination sign-in location. Acceptance in `Approvals > Returns` is the terminal action — it auto-signs the asset back in immediately. Pending returns are only visible in `Approvals > Returns`.
- `Check-out/In > Returns` is a read-only monitoring surface. It shows returns that have been accepted (completed) and any pending unaccepted returns for awareness. No sign-in action exists here.
- Direct intake via `Check-out/In > Standard` bypasses both surfaces and is used for unplanned walk-up returns only where no prior return request exists.
Locked intent:
...
- `Stationed` sign-in supports three outcomes: `Stationed` (asset returns to its resting state at the site), `Available` (asset is released from stationed assignment entirely), and `Damaged` (triggers the standard damage lock workflow)
- When `Damaged` is selected on a stationed sign-in, the responsible user from the temporary use assignment is locked and the full damage lock workflow from Section 10 applies without modification
...
- When a partial kit return is processed, each returned item is signed in with an individual outcome of `Available` or `Damaged`, chosen per item
- Unreturned items remain individually `Assigned` to the responsible user from the deployment
- Kit group tracking ends at deployment — after that, individual asset states apply
- The kit definition in `Settings > Kits` carries no deployment status
- For unreturned items, the manager can swap the asset out with another asset or choose to leave it unresolved; redeployment of the kit is not blocked
- `Returns` is a read-only monitoring surface showing pending and completed returns; no sign-in action exists here
...
- Permanent reassignment request goes to:
  - current holder (blocking approval — a decline moves the request to `Declined`)
  - admin (notified; can override a declined permanent reassignment with an explicit force-approve action)
- The asset manager at the current holder's location is notified of the request but has no approval role.
- The reassigned asset is treated as `Traveling` from the point of reassignment; no separate location-change step is needed.
...
- locked user
- responsible manager (defined as the active asset manager at the locked user's home-base location)
- admin

Fallback routing:
- If no active asset manager exists at the locked user's home-base location, all notifications route to admins only.
- If the user is in an `Unassigned` home-base state, all notifications route to admins only.
- The locked user's ability to submit the damage form and auto-unlock is not affected by manager routing.
...
## 15. Open Areas Still To Be Defined
These still need question rounds before the final baseline:
- exact fields on every page
- exact forms for:
  ...
  - Approvals
  - Settings tabs
- final field lists for forms/tabs and final status wording cleanup by view
- notification timing details by role if needed
- any remaining backend workflow edge cases
...
- Staff browse full inventory catalog and can request any asset on the system regardless of location
...
- `Returns` panels show return request details plus planned intake context.
...
- `Accept Return` in `Approvals > Returns` is the terminal action and auto-signs items back in immediately using the approver-selected location.
- `Check-out/In > Returns` is a read-only monitoring surface showing pending and completed returns; no sign-in action exists there.
...
- All in-flight workflows tied to the retiring location are automatically moved to `Blocked` status with a system reason of "Location retired." This includes: pending or in-progress asset requests, active sign-outs, open approval items, pending or in-progress transfers, and unresolved damage cases.
- All assets tied to the retiring location move to `Unassigned`.
- All users with that location as their home base move to an `Unassigned` home-base state.
- Affected users, managers, and admins are notified.
- Admin must resolve each asset and user individually before they become available again — assets must be assigned a new location and users must be assigned a new home base before they can resume workflows.
...
- Force-retired locations immediately move all assets to `Unassigned` and all users to an `Unassigned` home-base state; all in-flight workflows tied to the location are set to `Blocked` with reason "Location retired"; admin must resolve each asset and user before they become available again.
...
- The request basket auto-saves as a `Draft` at each step of the guided flow
- Drafts appear in `Requests > History` filtered to `Draft` status and are visible only to the requesting user
- Navigating away does not discard the draft
...
- Drafts in `Requests > History` are not visible in any manager or admin approval queue.
