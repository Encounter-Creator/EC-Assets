...
function getResetRedirectUrl() {
  return Linking.createURL("login", { scheme: "assetsapp" });
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadAccessContext = useCallback(
    async (userId: string) => {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then((result: any) => {
      const { data } = result;
      if (!mounted) return;
    ...
    () => ({
      ...
      signUp: async (email, password) => {
        ...
        const { data, error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: getResetRedirectUrl(),
            data: {
